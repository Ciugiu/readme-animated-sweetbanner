...
export function useBannerConfig() {
    ...
    const [selectedMeteor, setSelectedMeteor] = useState<string | null>(
        config.meteors[0]?.id ?? null
    );
                    ...
                    if (base64) {
                        setConfig((prev) => ({
                            ...
                            meteors: prev.meteors.map((m) =>
                                m.id === meteor.id ? { ...m, iconBase64: base64 } : m
                            ),
                        }));
                    }
                });
    ...
    const updateMeteor = useCallback(
        (id: string, updates: Partial<MeteorConfig>) => {
            setConfig((prev) => ({
                ...prev,
                meteors: prev.meteors.map((m) =>
                    m.id === id
    ...
    const addMeteor = useCallback(() => {
        ...
        const wouldOverlap = (
            ...
            const maxCycle = Math.max(
                ...meteors.map(m => m.delay + m.duration),
                candidateDelay + candidateDuration
            );
            ...
            for (let t = 0; t < checkDuration; t += timeStep) {
                const candidatePos = getMeteorPosition(
                    candidateX,
                    candidateAngle,
                    candidateDuration,
                    candidateDelay,
                ...
                for (const meteor of meteors) {
                    const meteorPos = getMeteorPosition(
                        meteor.startX,
                        meteor.angle,
                        meteor.duration,
                        meteor.delay,
        ...
        const maxDelay = meteors.length > 0
            ? Math.max(...meteors.map(m => m.delay)) + delayStep
            : 0;
        ...
        const generateRandomDelay = (): number => {
            ...
            for (let attempt = 0; attempt < 20; attempt++) {
                ...
                const tooClose = existingDelays.some(ed => Math.abs(ed - delay) < 1.5 || Math.abs(ed - delay - maxPeriod) < 1.5);
                if (!tooClose || attempt === 19) {
    ...
    const selectedMeteorData = config.meteors.find(
        (m) => m.id === selectedMeteor
    );
